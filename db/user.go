...
func Insuserfromfile() {
	...
	qry := "INSERT INTO user (eaddr, username,pass,sess_id,sess_exp,propername,teamassign,psnid,platform) VALUES(?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE username=?, pass=?, propername=?, psnid=?"
