	...
	for {
		...
		*/
		...
		if now.Sub(t) < time.Minute*30 {
			log.Printf("pulled data in: %v, time diff: %v, sleeping: %d secs ", time.Since(now), now.Sub(t), 10)
			time.Sleep(time.Second * 10)
		} else {
			log.Printf("pulled data in: %v, time diff: %v, sleeping: %d secs ", time.Since(now), now.Sub(t), 30)
